...
async fn main()
{
    ...
    let url_str = String::from("wss://stream.binance.com/stream?streams=bnbusdt@aggTrade/btcusdt@markPrice");
            ...
            Ok(Message::Text(text)) => {
                ...
                if msg_type == "aggTrade" {
                    ...
                    println!(
                        ...
                        msg_type, agg_trade.data.s, agg_trade.data.p, agg_trade.data.q
                    );
                } else if msg_type == "markPriceUpdate" {
                    let mark_price: MarkPriceUpdate = serde_json::from_str(&text).unwrap();
                    println!(
                        "Msg: {}, Symbol: {}, Price: {}, Rate: {}",
                        msg_type, mark_price.data.s, mark_price.data.p, mark_price.data.r
                    );
                }
            }
            Ok(Message::Ping(_))
